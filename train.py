...
if __name__ == '__main__':
    # 加载模型
    model = YOLO('yolo11s.pt')  # 不使用预训练权重训练
    # model = YOLO(r'yolov8.yaml').load("yolov8n.pt")  # 使用预训练权重训练
    # 训练参数 ----------------------------------------------------------------------------------------------
    model.train(
        data=r'C:\Users\admin\Desktop\yolo\picture\YOLODataset\dataset.yaml',
        epochs=500,  # (int) 训练的周期数
        patience=300,  # (int) 等待无明显改善以进行早期停止的周tiao期数
        ...
        lrf=0.003,  # (float) 最终学习率（lr0 * lrf）
        momentum=0.9,  # (float) SGD动量/Adam beta1
        ...
        box=5.0,  # (float) 盒损失增益
        cls=3.0,  # (float) 类别损失增益（与像素比例）
        dfl=1.5,  # (float) dfl损失增益
        pose=5.0,  # (float) 姿势损失增益
        ...
        fliplr=0.3,  # (float) 图像左右翻转（概率）
        mosaic=0.2,  # (float) 图像马赛克（概率）
